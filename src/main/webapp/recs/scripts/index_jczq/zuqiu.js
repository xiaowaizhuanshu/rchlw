...
//当点击"胆"的时候，全部改变按钮的颜色（样式）
function changeColorCheckbox(obj,day, stat, week) {
	if($(obj).attr("class")=='bat_dan'){
		$(obj).attr("class","bat_dan_on");
	}else{
...
//当点击按钮的时候，改变按钮的颜色（样式），同时检查按钮是否全部选定，
function checkColorButton(object, day, type, stat,week) {
	//var obj = $("#ball" + type + "_" + day + "_" + stat);
	if ($(object).attr("class") == "bat_sheng") {
		$(object).removeClass("bat_sheng").addClass("bat_ping");
	} else{
		$(object).removeClass("bat_ping").addClass("bat_sheng");
	}
	checkZhuma();//获取我选择的数据
...
//	});
	...
	for (var i = 1; i <= count; i++) {
			var spfStr = "";//记录胜负平
			//如果胜负平随意一项被选择，则记录日期，周几，编号
				if ($("#ball0_" + day + "_" + i).attr("class") == "bat_ping" || $("#ball1_" + day + "_" + i).attr("class") == "bat_ping" || $("#ball3_" + day + "_" + i).attr("class") == "bat_ping") {
					matchDate = $("#day_" + day + "_" + i).val();
					matchWeek = $("#weekid_" + day + "_" + i).val();
					match = $("#no_" + day + "_" + i).html();
				}
				if ($("#ball0_" + day + "_" + i).attr("class") == "bat_ping") {
					spfStr = spfStr+"0";
				}
				if ($("#ball1_" + day + "_" + i).attr("class") == "bat_ping") {
					spfStr = spfStr+"1";
				}
				if ($("#ball3_" + day + "_" + i).attr("class") == "bat_ping") {
					spfStr = spfStr+"3";
				}
				//加定胆标识："$"
				if($("#dingdan_"+day+"_"+i).attr("checked")){//如果有加定胆
					if(zhuma.indexOf("$")>-1){
...
function addMatchForRight(object, day, a, b,week) {
	if (object.attr("class") != "bat_ping") {//如果没有选中
		//如果只选中了胜负平其中的一项
		...
		if (allMatch > 10) {
			...
			var no = $("#no_" + day + "_" + a).html();//获取编号
			var homeTeam = $("#hteam_" + day + "_" + a).html();//获取主队的内容，连带着a标签
