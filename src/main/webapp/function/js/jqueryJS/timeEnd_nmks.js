...
function initKaiJiangTime(){//结期时间和距离开奖间隔时间s(不稳)
	var temper= $("#kaijiangtime").text();
	sysTime = $("#sysTime").val();
	var tms = [];
	if(temper == null || temper ==''){
		tms[0] = 0;
	}else{
//		var dt = new Date(temper);
//		var td = dt.getTime();
//		alert("td:"+td);
		tms[0] =temper - sysTime;
	}
	//计算天、时、分、秒、
	var days = Math.floor(tms[0] / (1000 * 60 * 60 * 24));
	var hours = Math.floor(tms[0] / (1000 * 60 * 60)) % 24;
	...
	if (seconds < 0)seconds = 0;
	//将天、时、分、秒插入到html中
	$("#kaijiangday").html(days);
	$("#kaijianghour").html(hours);
	$("#kaijiangminute").html(minutes);
	$("#kaijiangsecond").html(seconds);
}
...
function takeCount(){
	...
	if (seconds < 0)
		...
		if(hours == 0 && minutes == 0 && seconds==0){
			...
			getGaoPinBatchCode('F47107');
			reHtml(45,'lotno=F47107',false,'ChannelBuyBulletin','true');
