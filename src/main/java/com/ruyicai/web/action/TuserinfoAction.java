		...
		try { 
			...
			if(CommonUtil.getBackValue("errorCode",re).equals(LotErrorCode.NEW_OK)){
				...
				if("0".equals(reerrcode)){
					...
					if(!(" ".equals(user.getEMAIL())&&user.getMOBILEID()==null&&"111111111111111111".equals(user.getCERTID())&&" ".equals(user.getNAME())&&" ".equals(user.getNICKNAME()))&&count<1){
...
/**
		...
		try {
			JSONObject obj = JSONObject.fromObject(
					JSONReslutUtil.getResultMessage(
							ResourceBundleUtil.LINKURL + "/select/getCash?","id="+id,"GET"));
			...
			if(obj!=null && CommonUtil.getBackValue("errorCode", obj).equals(ErrorCode.OK.value)){
				 ...
				 if("null".equals(obj.getString("value")) || obj.getString("value")==null || "".equals(obj.getString("value"))){
					 	logger.info("根据新接口getCash和订单号id:"+id+"未查询到提现详细信息");
					 	request.setAttribute("list", list);
				 }else{
					//开始处理时间格式
						if(oneobj.get("plattime")!=null && !oneobj.get("plattime").toString().equals("null") && !oneobj.get("plattime").toString().equals("")){
							oneobj.put("plattime", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(oneobj.getLong("plattime"))));
						}
						if(oneobj.get("checktime")!=null && !oneobj.get("checktime").toString().equals("null") && !oneobj.get("checktime").toString().equals("")){
							oneobj.put("checktime", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(oneobj.getLong("checktime"))));
						}			
						if(oneobj.get("remittime")!=null && !oneobj.get("remittime").toString().equals("null") && !oneobj.get("remittime").toString().equals("")){
							oneobj.put("remittime", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(oneobj.getLong("remittime"))));
						}			
						if(oneobj.get("finishtime")!=null && !oneobj.get("finishtime").toString().equals("null") && !oneobj.get("finishtime").toString().equals("")){
							oneobj.put("finishtime", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(oneobj.getLong("finishtime"))));
						}			
						if(oneobj.get("estimatefinishtime")!=null && !oneobj.get("estimatefinishtime").toString().equals("null") && !oneobj.get("estimatefinishtime").toString().equals("")){
							oneobj.put("estimatefinishtime", new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date(oneobj.getLong("estimatefinishtime"))));
						}
	...
	public String selectCashState(){
		try {
			...
			String endTime = request.getParameter("endTime")==null?new SimpleDateFormat("yyyy-MM-dd").format(now):request.getParameter("endTime");
			...
			JSONObject obj = JSONObject.fromObject(
					JSONReslutUtil.getResultMessage(
							ResourceBundleUtil.LINKURL + "/select/getCashs?","userno="+user.getUSERNO()+"&beginTime="+beginTime+"&endTime="+endTime+"&startLine="+0+"&endLine="+lineNum,"GET"));
	...
	public String cash(){
		...
		try {
			...
			logger.info("要提现信息为:金额:"+cashBean.getWithdrawel_money()+";银行账号:"+cashBean.getID_number()
			...
			logger.info("执行提现："+ResourceBundleUtil.LINKURL + "/taccounts/drawCash?"
					+"userno="+user.getUSERNO()
					+"&balance="+withdrawel+
				"&bankAccount="+cashBean.getID_number()+"&name="+URLEncoder.encode(cashBean.getReal_name())+"&bankId="+"0000001"
							+"&BANKNAME="+URLEncoder.encode(cashBean.getBank_name()));
			//调用 新接口执行提现内容
			JSONObject obj = JSONObject.fromObject(
							JSONReslutUtil.getResultMessage(
...
public void phoneBandSendMessage(){
	...
	response.setContentType("text/html;charset=utf-8");
	try {
		String usermobile=request.getParameter("usermobile").equals("undefined")?
				(String)session.getAttribute("new_phone"):request.getParameter("usermobile");
		...
		if(count>=3){
			...
			JSONObject jsonSMS = JSONObject.fromObject(JSONReslutUtil.getResultMessage(
					ResourceBundleUtil.USERSCENTERURL +"user/phonebind!sendMsgForPhoneBind;jsessionid="+jsessionid+"?", 
					"new_phone="+usermobile, "POST"));
	...
	public String accountDetail(){
		try {
			...
			String enddate = request.getParameter("endTime")==null?df.format(new Date()):request.getParameter("endTime");;
 ...
 */
	public void getParam(){
		try{
			...
			logger.info("提现参数2username="+username+"real_name="+request.getParameter("real_name")+"bank_name="+request.getParameter("bank_name"));
...
//			e.printStackTrace();
	...
	public String trackingNumberSelectAll(){
		try{
			...
			String startDate = request.getParameter("beginTime");
			String stopDate = request.getParameter("endTime")==null?df.format(new Date()):request.getParameter("endTime");;
	...
	public  String  trackingNumberInfo() {
		...
		try{
			...
			String moreMessage = "";
			if(lotArray.size()>5){
				moreMessage = "<a href='/chwl/function/rules/moreBetInfo.jsp?orderid="+ orderId +"' title='显示全部内容'>显示全部内容</a>";
			}else{
				moreMessage = "";
			}
		...
		if(lotno.equals(Constant.NMKS)||lotno.equals(SSCConstant.SSC)||lotno.equals(Constant.SYXW)||lotno.equals(Constant.SYYDJ)){
			if(isShouyilu == 1){
...
//					String url =ResourceBundleUtil.LINKSCOREURL + "/findScoreDetailByUserno?userno=" + userno +"&startLine="+ (pageIndex-1)*pageNum +"&endLine=" + pageNum+"&condition=" + condition.toString();
					JSONObject obj=JSONObject.fromObject(JSONReslutUtil.getResultMessage(ResourceBundleUtil.LINKSCOREURL + 
							"/findScoreDetailByUserno?" ,  "userno=" + user.getUSERNO() +"&startLine="+ (pageIndex-1)*pageNum +"&endLine=" + pageNum+"&condition=" + condition.toString(),
							"POST"));
					JSONArray TodyList = new JSONArray();//今天的记录
					JSONArray YestdayList = new JSONArray();//昨天的记录
					JSONArray MoreOldList = new JSONArray();//更早的记录
					...
					for (int i = 0; i < arrList.size(); i++) {  
						...
						if( "null".equals(rec.getString("currentScore"))){
							JSONObject	objs =JSONObject.fromObject(JSONReslutUtil.getResultMessage(ResourceBundleUtil.LINKSCOREURL + "/findScoreByUserno?","userno=" + user.getUSERNO(),"POST"));
			...
			try{
				...
				logger.info("发送给用户的相关信息是：用户编号"+user.getUSERNO());
					String url =ResourceBundleUtil.LINKSCOREURL + "/findScoreByUserno?userno=" + user.getUSERNO();
					 obj=JSONObject.fromObject(JSONReslutUtil.getResultMessage(ResourceBundleUtil.LINKSCOREURL + "/findScoreByUserno?","userno=" + user.getUSERNO(),"POST"));
		...
		public void transScore2Money() throws IOException{
			...
			try{
				...
				if(user.getUSERNO() == null){//未登录提示
					...
					String url =ResourceBundleUtil.LINKSCOREURL + "/transScore2Money?userno=" + user.getUSERNO()+"&score="+scroe;
					logger.info("积分兑换调用接口："+url);
					JSONObject obj=JSONObject.fromObject(JSONReslutUtil.getResultMessage(ResourceBundleUtil.LINKSCOREURL + "/transScore2Money?","userno=" +  user.getUSERNO()+"&score="+scroe,"POST"));
					logger.info("recivePresent 返回结果"+obj);
					if(!LotErrorCode.NEW_OK.trim().equals(obj.getString("errorCode").trim())){
