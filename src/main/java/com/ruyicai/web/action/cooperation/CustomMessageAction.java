	...
	public String Custom() {
		try {
			...
			if (!request.getParameter("china").equals("中文")) {
				...
				useremail = URLEncoder.decode(request.getParameter("useremail").equals("null")
						|| request.getParameter("useremail").length() == 0 ? user.getString("email") == null ? ""
						: user.getString("email") : request.getParameter("useremail"));
				userphone = URLEncoder.decode(request.getParameter("userphone").equals("null")
						|| request.getParameter("userphone").length() == 0 ? user.getString("mobileid") == null ? ""
						: user.getString("mobileid") : request.getParameter("userphone"));
			} else {
				questiontype = request.getParameter("select");
				contents = request.getParameter("contents");
				useremail = request.getParameter("useremail").equals("null")
						|| request.getParameter("useremail").length() == 0 ? user.getString("email") == null ? ""
						: user.getString("email") : request.getParameter("useremail");
				userphone = request.getParameter("userphone").equals("null")
						|| request.getParameter("userphone").length() == 0 ? user.getString("email") == null ? ""
						: user.getString("email") : request.getParameter("userphone");
			}
			JSONObject obj = JSONObject.fromObject(CustomMessage.getCustom(null, 0, usertime, 1, questiontype,
					contents, useremail, userphone, ipStr));
	...
	public String getMsgList() {
		try {
			...
			Integer pageNum = (request.getParameter("pageCount") == null || request.getParameter("pageCount").equals(
					"undefined")) ? 5 : Integer.valueOf(request.getParameter("pageCount"));//页数
			//得到传入条件
			...
			if (userinfojson != null && !LotErrorCode.YHBCZ.trim().equals(userinfojson.getString("errorCode").trim())) {
				conditionMap.put("EQS_userno", userno);
			} else {
				conditionMap.put("EQS_flag", "1");
			}
			String url = ResourceBundleUtil.LINKMESSAGE + "/selectMsgs?startLine=" + (pageIndex - 1) * pageNum
					+ "&endLine=" + pageNum + "&condition=" + conditionMap.toString();
			JSONObject obj = JSONObject.fromObject(
					JSONReslutUtil.getResultMessage(ResourceBundleUtil.LINKMESSAGE + "/selectMsgs",
							"startLine=" + (pageIndex - 1) * pageNum + "&endLine=" + pageNum + "&condition="
									+ conditionMap.toString(), "POST")).getJSONObject("value");
			...
			if (!obj.isNullObject()) {
				...
				String pageHtml = CommonUtil.getMesgToJsp(pageIndex, Integer.parseInt(obj.getString("totalResult")),
						pageNum, 4, "list");
