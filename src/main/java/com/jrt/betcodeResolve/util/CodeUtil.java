...
public class CodeUtil {
	 ...
	 */
	public static JSONObject getSSQCodeString(String betCode,String multiple){
	...
	public static JSONObject getCodeString(String lotNo,String betCode,String lotmulti){
		...
		try {
			...
			if(lotNo.equals(Constant.SSQ) || lotNo.equals(Constant.SSQ_DM)){
				jsonObject = getSSQCodeString(betCode,lotmulti);
				return jsonObject;
			//福彩3D
			...
			//内蒙快三
				...
				jsonObject = getNMKSCodeString(betCode,lotNo);
				return jsonObject;
			...
			//体彩
					...
					|| lotNo.equals(Constant.SYXW) || lotNo.equals(Constant.SYXW_DM) || lotNo.equals(Constant.SYYDJ )
					||lotNo.equals(Constant.EEXW) ||lotNo.equals(Constant.SYXW_HH)){
	 ...
	 * @param betcode
	 * 注码格式：
	 * 000005010305^000005010305^
	 * 010001010105^010001010105^
	 * 020001060606^020001060606^
	 * 	10000203040917^
	 * 	2000030412233414^
	 * 	21000103010203^
	 * 	220001 01*020304^
	 * 	300001050102030405^
	 * 	400001^
	 * 	630001 06 010203040506^
	 * 	640001 0102*03040506^
	 * 	710001 0203*0405^
	 *  810001 06 010203040506^
	 * @return
	 */
	public static JSONObject getNMKSCodeString(String betcode,String lotNo) {
		JSONObject obj = new JSONObject();
		String betcodearr[] = betcode.split("\\^");
		String code="";
		String playtype = "";
			for(String betcode1 : betcodearr){
				playtype = betcode1.substring(0, 2);
				String multiple = betcode1.substring(4,6);
				obj.put("multiple", multiple);
				if (betcode1.indexOf("*") > -1) {
					String arr[] = betcode1.substring(6,betcode1.length()).split("\\*");
					obj.put("danma",joinStringArray(getStringArray(arr[0])));
					obj.put("tuoma",joinStringArray(getStringArray(arr[1])));
				}else{
					if(playtype.equals("20")||playtype.equals("30")||playtype.equals("63")||playtype.equals("21")||playtype.equals("10")||playtype.equals("81")){
						code  += joinStringArray(getStringArray(betcode1.substring(8,betcode1.length())))+"<br/>";
					}else if(playtype.equals("50")||playtype.equals("40")){
						code = getWanfaByPlayType(playtype, Constant.NMKS);
					}
					else{
						code  +=joinStringArray(getStringArray(betcode1.substring(6,betcode1.length()))) +"<br/>";
					}
				}
			}
		obj.put("wanfa",playtype);
		obj.put("betcode",code);
        return obj;
	}
	 ...
	 * @return
	 */
	public static String joinStringArray(List<String> v) {
		String resultStr="";
		if (v==null||v.size()==0) {
			return "";
		} else {
	 ...
	 * @return
	 */
	public static List<String> getStringArray(String strArray) {
		try {
	 ...
	 * @return
	 */
	public static String getWanfaByPlayType(String playtype,String lotno) {
		String wanfa = "";
		if(lotno.equals(Constant.NMKS)){
			if(playtype.equals(Constant.NMKS_HZ)){
				wanfa = "和值";
			}else if(playtype.equals(Constant.NMKS_STHTX)){
				wanfa = "三同号通选";
			}else if(playtype.equals(Constant.NMKS_STDS)){
				wanfa = "三同号单选";
			}else if(playtype.equals(Constant.NMKS_STMS)){
				wanfa = "三同号复式";
			}else if(playtype.equals(Constant.NMKS_ERTHMS)){
				wanfa = "二同号复式";
			}else if(playtype.equals(Constant.NMKS_ERTDS)){
				wanfa = "二同号单式";
			}else if(playtype.equals(Constant.NMKS_ERTZH)){
				wanfa = "二同号组合";
			}else if(playtype.equals(Constant.NMKS_ERBT)){
				wanfa = "二不同号";
			}else if(playtype.equals(Constant.NMKS_ERBTZH)){
				wanfa = "二不同号组合";
			}else if(playtype.equals(Constant.NMKS_ERBTHDT)){
				wanfa = "二不同号胆拖";
			}else if(playtype.equals(Constant.NMKS_SBTHDS)){
				wanfa = "三不同号单式";
			}else if(playtype.equals(Constant.NMKS_SBTHMS)){
				wanfa = "三不同号复式";
			}else if(playtype.equals(Constant.NMKS_SBTHDT)){
				wanfa = "三不同号胆拖";
			}else if(playtype.equals(Constant.NMKS_SLHTX)){
				wanfa = "三连号通选";
			}
		}
        return wanfa;
		...
		if (betcode.indexOf(";") > -1) {
			...
			if (lotNo.equals(Constant.PLS) || lotNo.equals(Constant.PLS_DM)) {
				...
				if("大小单双".equals(obj.getString("wanfa"))){
					String code = array[1].replace("1","大").replace("2","小").replace("4","双").replace("5","单");
					obj.put("betcode", code+"<br/>");
				}else{
			...
			//十一选五	
				...
				obj.put("multiple", multiple);
				//十一运夺金
			}else if(lotNo.equals(Constant.SYXW_HH)){
				if (betcode.indexOf("|") > -1) {
					String arr[] = betcode.split("\\|");
					obj.put("betcode",arr[2].replace(";", ""));
				}else{
					obj.put("betcode",betcode.replace(";", ""));
				}
				obj.put("wanfa",get11xuan5hhWanfa(betcode,lotNo).get("wanfa"));
				obj.put("multiple", multiple);
				//十一运夺金
			}else if(lotNo.equals(Constant.SYYDJ )){
				if (betcode.indexOf("@") > -1) {
...
/*	public static String getZCBetcode(String betcode) {
	 ...
	 * 
	 */
	public static JSONObject getSelectNum(String betcode,int chooseNum){
		...
		for(int i=0;i<bets.length;i++){
			...
			else if(bets[i].length()==3){threeselect++;}
			else if(chooseNum == 4 && bets[i].length() == 4){allselect++;}
		}
		selectNum.put("radio",radio);
		selectNum.put("twoselect",twoselect);
		selectNum.put("threeselect",threeselect);
		selectNum.put("allselect", allselect);
		return selectNum;
	}
	 ...
	 * @return
	 */
	public static JSONObject get11xuan5hhWanfa (String betCode,String lotno){
		//s|R2|020304
				...
				if("T01014".equals(lotno)){
					if(betCode.indexOf(";")>-1){
						...
						if(code[0].indexOf("|")>-1){
							...
							String s = "";
							if( arr[0].equals("S")){s = "单式";}else if (arr[0].equals("M")){s = "复式";}
							if("R1".equals(arr[1])){
								zhuma.put("wanfa","任选一"+s);
							}else if("R2".equals(arr[1])){
								 if(arr[1].indexOf("$")>-1){
								  zhuma.put("wanfa","任选二胆拖"+s);
								 }else{
								   zhuma.put("wanfa","任选二"+s);
								 }
							}else if("R3".equals(arr[1])){
								 if(arr[1].indexOf("$")>-1){
									  zhuma.put("wanfa","任选三胆拖"+s);
									  //zhuma.put("code", )
									 }else{
									   zhuma.put("wanfa","任选三"+s);
									 }
							}else if("R4".equals(arr[1])){
								 if(arr[1].indexOf("$")>-1){
									  zhuma.put("wanfa","任选四胆拖"+s);
									  //zhuma.put("code", )
									 }else{
									   zhuma.put("wanfa","任选四"+s);
									 }
							}else if("R5".equals(arr[1])){
								 if(arr[1].indexOf("$")>-1){
									  zhuma.put("wanfa","任选五胆拖"+s);
									  //zhuma.put("code", )
									 }else{
									   zhuma.put("wanfa","任选五"+s);
									 }
							}else if("R6".equals(arr[1])){
								 if(arr[1].indexOf("$")>-1){
									  zhuma.put("wanfa","任选六胆拖"+s);
									  //zhuma.put("code", )
									 }else{
									   zhuma.put("wanfa","任选六"+s);
									 }
							}else if("R7".equals(arr[1])){
								 if(arr[1].indexOf("$")>-1){
									  zhuma.put("wanfa","任选七胆拖"+s);
									  //zhuma.put("code", )
									 }else{
									   zhuma.put("wanfa","任选七"+s);
									 }
							}else if("R8".equals(arr[1])){
								 if(arr[1].indexOf("$")>-1){
									  zhuma.put("wanfa","任选八胆拖"+s);
									  //zhuma.put("code", )
									 }else{
									   zhuma.put("wanfa","任选八"+s);
									 }
							}else if("Z3".equals(arr[1])){
								 if(arr[1].indexOf("$")>-1){
									  zhuma.put("wanfa","前三组胆"+s);
									  //zhuma.put("code", )
									 }else{
									   zhuma.put("wanfa","前三组选"+s);
									 }
							}else if("Z2".equals(arr[1])){
								 if(arr[1].indexOf("$")>-1){
									  zhuma.put("wanfa","前二组胆"+s);
									  //zhuma.put("code", )
									 }else{
									   zhuma.put("wanfa","前二组选"+s);
									 }
							}else if("Q2".equals(arr[1])){
									 zhuma.put("wanfa","前二直选"+s);
									
							}else if("Q3".equals(arr[1])){
									 zhuma.put("wanfa","前三直选"+s);
 ...
 */
	 ...
	 */
	   ...
	   * @return
	   * 
	   * 	public static final String BD_SPF = "B00001";//	北单胜平负  B00001
	public static final String BD_ZJQ = "B00002";//	北单总进球  B00002
	public static final String BD_BQC = "B00003";//	北单半全场  B00003
	public static final String BD_SXPDS = "B00004";//	北单上下盘单双 B00004
	public static final String BD_DCBF = "B00005";//	北单单场比分  B00005
	   */
	  
	  public static String getBDTouzhu(String code,String lotno){
		  String  codeString ="";
		  if(Constant.BD_DCBF.equals(lotno)){
		    ...
		    } else if(Constant.BD_SPF.equals(lotno)){//北单胜平负
			  ...
			  }else if(Constant.BD_BQC.equals(lotno)){//北单半全场 
				  for(int i=1;i<code.split("").length;i+=2){
			  ...
			  }else if(Constant.BD_SXPDS.equals(lotno)){//北单上下盘单双
				  for(int i=1;i<code.split("").length;i++){
					  if("1".equals(code.split("")[i])){
						  codeString+="上单";
					  }else if("2".equals(code.split("")[i])){
						  codeString+="上双";
					  }else if("3".equals(code.split("")[i])){
						  codeString+="下单";
					  }else if("4".equals(code.split("")[i])){
						  codeString+="下双";
					  }
					  codeString+=",";
				  }
			  }else if(Constant.BD_ZJQ.equals(lotno)){//北单总进球
				  for(int i=1;i<code.split("").length;i++){
					  if(!"7".equals(code.split("")[i])){
						  codeString +="进"+code.split("")[i]+"球";
					  }else{
						  codeString +="进7+球";
					  }
					  codeString+=",";
	  ...
	  public static String getLQTouzhu(String code,String lotno){
		  ...
		  }else if(Constant.JCLQ_SF.equals(lotno)||Constant.JCLQ_RFSF.equals(lotno)||Constant.JCZQ_RSPF.equals(lotno)||Constant.JCZQ_SPF.equals(lotno)){//竞彩篮球足球 胜（平）负
			  for(int i=1;i<code.split("").length;i++){
				  if("3".equals(code.split("")[i])){
					  codeString+="胜";
				  }else if("0".equals(code.split("")[i])){
